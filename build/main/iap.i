...
# 19 "C:/esp/esp-idf/components/app_update/include/esp_ota_ops.h" 2
# 1 "c:\\msys32\\opt\\xtensa-esp32-elf\\lib\\gcc\\xtensa-esp32-elf\\5.2.0\\include\\stdbool.h" 1 3 4
# 20 "C:/esp/esp-idf/components/app_update/include/esp_ota_ops.h" 2
# 1 "c:\\msys32\\opt\\xtensa-esp32-elf\\lib\\gcc\\xtensa-esp32-elf\\5.2.0\\include\\stddef.h" 1 3 4
...
# 22 "C:/esp/esp-idf/components/app_update/include/esp_ota_ops.h" 2
...
esp_err_t esp_partition_mmap(const esp_partition_t* partition, uint32_t offset, uint32_t size,
                             ...
                             const void** out_ptr, spi_flash_mmap_handle_t* out_handle);
# 23 "C:/esp/esp-idf/components/app_update/include/esp_ota_ops.h" 2
# 1 "C:/esp/esp-idf/components/spi_flash/include/esp_spi_flash.h" 1
# 24 "C:/esp/esp-idf/components/app_update/include/esp_ota_ops.h" 2
...
iap_err_t iap_init()
{
    if (((esp_log_level_t) 3) >= ESP_LOG_DEBUG) { esp_log_write(ESP_LOG_DEBUG, "iap", "D" " (%d) %s: " "iap_init" "\033[0m" "\n", esp_log_timestamp(), "iap"); };


    if (iap_state.module_state_flags & (1 << 0)) {
        if (((esp_log_level_t) 3) >= ESP_LOG_ERROR) { esp_log_write(ESP_LOG_ERROR, "iap", "\033[0;" "31" "m" "E" " (%d) %s: " "iap_init: The module has already been initialized!" "\033[0m" "\n", esp_log_timestamp(), "iap"); };
        return 0x101;
    }

    iap_state.module_state_flags = (1 << 0);
...
iap_err_t iap_begin()
{
    if (((esp_log_level_t) 3) >= ESP_LOG_DEBUG) { esp_log_write(ESP_LOG_DEBUG, "iap", "D" " (%d) %s: " "iap_begin" "\033[0m" "\n", esp_log_timestamp(), "iap"); };


    if (!(iap_state.module_state_flags & (1 << 0))) {
        if (((esp_log_level_t) 3) >= ESP_LOG_ERROR) { esp_log_write(ESP_LOG_ERROR, "iap", "\033[0;" "31" "m" "E" " (%d) %s: " "iap_begin: the module hasn't been initialized!" "\033[0m" "\n", esp_log_timestamp(), "iap"); };
        return 0x102;
    }


    if (iap_state.module_state_flags & (1 << 1)) {
        if (((esp_log_level_t) 3) >= ESP_LOG_ERROR) { esp_log_write(ESP_LOG_ERROR, "iap", "\033[0;" "31" "m" "E" " (%d) %s: " "iap_begin: Session already open!" "\033[0m" "\n", esp_log_timestamp(), "iap"); };
        return 0x103;
    }


    iap_state.page_buffer_ix = 0;
    iap_state.page_buffer = malloc(4096);
    if (!iap_state.page_buffer) {
        if (((esp_log_level_t) 3) >= ESP_LOG_ERROR) { esp_log_write(ESP_LOG_ERROR, "iap", "\033[0;" "31" "m" "E" " (%d) %s: " "iap_begin: not enough heap memory to allocate the page buffer!" "\033[0m" "\n", esp_log_timestamp(), "iap"); };
        return 0x104;
    }

    iap_state.partition_to_program = iap_find_next_boot_partition();
    if (!iap_state.partition_to_program) {
        if (((esp_log_level_t) 3) >= ESP_LOG_ERROR) { esp_log_write(ESP_LOG_ERROR, "iap", "\033[0;" "31" "m" "E" " (%d) %s: " "iap_begin: partition for firmware update not found!" "\033[0m" "\n", esp_log_timestamp(), "iap"); };
        free(iap_state.page_buffer);
        return 0x106;
    }

    if (((esp_log_level_t) 3) >= ESP_LOG_DEBUG) { esp_log_write(ESP_LOG_DEBUG, "iap", "D" " (%d) %s: " "iap_begin: next boot partition is '%s'." "\033[0m" "\n", esp_log_timestamp(), "iap", iap_state.partition_to_program->label); };
    ...
    if (result != 0) {
        if (((esp_log_level_t) 3) >= ESP_LOG_ERROR) { esp_log_write(ESP_LOG_ERROR, "iap", "\033[0;" "31" "m" "E" " (%d) %s: " "iap_begin: esp_ota_begin failed (%d)!" "\033[0m" "\n", esp_log_timestamp(), "iap", result); };
        free(iap_state.page_buffer);
        return -1;
    }

    if (((esp_log_level_t) 3) >= ESP_LOG_INFO) { esp_log_write(ESP_LOG_INFO, "iap", "\033[0;" "32" "m" "I" " (%d) %s: " "iap_begin: opened IAP session for partition '%s', address 0x%08x." "\033[0m" "\n", esp_log_timestamp(), "iap", iap_state.partition_to_program->label, iap_state.cur_flash_address); }
                                                                                ;

    iap_state.module_state_flags |= (1 << 1);
    return 0;
}

iap_err_t iap_write(uint8_t *bytes, uint16_t len)
{
    if (((esp_log_level_t) 3) >= ESP_LOG_DEBUG) { esp_log_write(ESP_LOG_DEBUG, "iap", "D" " (%d) %s: " "iap_write(bytes = %p, len = %u)" "\033[0m" "\n", esp_log_timestamp(), "iap", bytes, len); };


    if (!(iap_state.module_state_flags & (1 << 0))) {
        if (((esp_log_level_t) 3) >= ESP_LOG_ERROR) { esp_log_write(ESP_LOG_ERROR, "iap", "\033[0;" "31" "m" "E" " (%d) %s: " "iap_write: the module hasn't been initialized!" "\033[0m" "\n", esp_log_timestamp(), "iap"); };
        return 0x102;
    }


    if (!(iap_state.module_state_flags & (1 << 1))) {
        if (((esp_log_level_t) 3) >= ESP_LOG_ERROR) { esp_log_write(ESP_LOG_ERROR, "iap", "\033[0;" "31" "m" "E" " (%d) %s: " "iap_write: programming session not open!" "\033[0m" "\n", esp_log_timestamp(), "iap"); };
        return 0x105;
    }

    if (((esp_log_level_t) 3) >= ESP_LOG_DEBUG) { esp_log_write(ESP_LOG_DEBUG, "iap", "D" " (%d) %s: " "iap_write: cur_flash_address = 0x%08x" "\033[0m" "\n", esp_log_timestamp(), "iap", iap_state.cur_flash_address); };

    while (len > 0) {

        uint16_t spaceRemaining = 4096 - iap_state.page_buffer_ix;
        uint16_t nofBytesToCopy = ((spaceRemaining) < (len) ? (spaceRemaining) : (len));
...
# 164 "C:/esp/ArkPOS2-esp/main/iap.c"
            ...
            if (result != 0) {
                if (((esp_log_level_t) 3) >= ESP_LOG_ERROR) { esp_log_write(ESP_LOG_ERROR, "iap", "\033[0;" "31" "m" "E" " (%d) %s: " "iap_write: write failed (%d)!" "\033[0m" "\n", esp_log_timestamp(), "iap", result); };
                return 0x107;
            }
        }
    }
...
iap_err_t iap_commit()
{
    if (((esp_log_level_t) 3) >= ESP_LOG_DEBUG) { esp_log_write(ESP_LOG_DEBUG, "iap", "D" " (%d) %s: " "iap_commit" "\033[0m" "\n", esp_log_timestamp(), "iap"); };

    iap_err_t result = iap_write_page_buffer();
    if (result != 0) {
        if (((esp_log_level_t) 3) >= ESP_LOG_ERROR) { esp_log_write(ESP_LOG_ERROR, "iap", "\033[0;" "31" "m" "E" " (%d) %s: " "iap_commit: programming session failed in final write." "\033[0m" "\n", esp_log_timestamp(), "iap"); };
    }

    result = iap_finish(1);
    if (result != 0) {
        if (((esp_log_level_t) 3) >= ESP_LOG_ERROR) { esp_log_write(ESP_LOG_ERROR, "iap", "\033[0;" "31" "m" "E" " (%d) %s: " "iap_commit: programming session failed in iap_finish." "\033[0m" "\n", esp_log_timestamp(), "iap"); };
    }

    if (((esp_log_level_t) 3) >= ESP_LOG_INFO) { esp_log_write(ESP_LOG_INFO, "iap", "\033[0;" "32" "m" "I" " (%d) %s: " "iap_commit: programming session successfully completed, partition activated." "\033[0m" "\n", esp_log_timestamp(), "iap"); };
    return result;
}

iap_err_t iap_abort()
{
    if (((esp_log_level_t) 3) >= ESP_LOG_DEBUG) { esp_log_write(ESP_LOG_DEBUG, "iap", "D" " (%d) %s: " "iap_abort" "\033[0m" "\n", esp_log_timestamp(), "iap"); };

    iap_err_t result = iap_finish(0);
    if (result == 0) {
        if (((esp_log_level_t) 3) >= ESP_LOG_INFO) { esp_log_write(ESP_LOG_INFO, "iap", "\033[0;" "32" "m" "I" " (%d) %s: " "iap_abort: programming session successfully aborted." "\033[0m" "\n", esp_log_timestamp(), "iap"); };
    }
...
static iap_err_t iap_write_page_buffer()
{
    if (((esp_log_level_t) 3) >= ESP_LOG_DEBUG) { esp_log_write(ESP_LOG_DEBUG, "iap", "D" " (%d) %s: " "iap_write_page_buffer" "\033[0m" "\n", esp_log_timestamp(), "iap"); };
    if (iap_state.page_buffer_ix == 0) {
        return 0;
    }

    if (((esp_log_level_t) 3) >= ESP_LOG_DEBUG) { esp_log_write(ESP_LOG_DEBUG, "iap", "D" " (%d) %s: " "iap_write_page_buffer: writing %u bytes to address 0x%08x" "\033[0m" "\n", esp_log_timestamp(), "iap", iap_state.page_buffer_ix, iap_state.cur_flash_address); }
                                                                   ;
    esp_err_t result = esp_ota_write(iap_state.ota_handle, iap_state.page_buffer, iap_state.page_buffer_ix);
    if (result != 0) {
        if (((esp_log_level_t) 3) >= ESP_LOG_ERROR) { esp_log_write(ESP_LOG_ERROR, "iap", "\033[0;" "31" "m" "E" " (%d) %s: " "iap_write_page_buffer: write failed in esp_ota_write (%d)!" "\033[0m" "\n", esp_log_timestamp(), "iap", result); };
        return 0x107;
    }
...
static iap_err_t iap_finish(int commit)
{
    ...
    if (!(iap_state.module_state_flags & (1 << 0))) {
        if (((esp_log_level_t) 3) >= ESP_LOG_ERROR) { esp_log_write(ESP_LOG_ERROR, "iap", "\033[0;" "31" "m" "E" " (%d) %s: " "iap_finish: the module hasn't been initialized!" "\033[0m" "\n", esp_log_timestamp(), "iap"); };
        return 0x102;
    }


    if (!(iap_state.module_state_flags & (1 << 1))) {
        if (((esp_log_level_t) 3) >= ESP_LOG_ERROR) { esp_log_write(ESP_LOG_ERROR, "iap", "\033[0;" "31" "m" "E" " (%d) %s: " "iap_finish: programming session not open!" "\033[0m" "\n", esp_log_timestamp(), "iap"); };
        return 0x105;
    }
...
# 244 "C:/esp/ArkPOS2-esp/main/iap.c"
    ...
    if (commit) {
        if (result != 0) {
            if (((esp_log_level_t) 3) >= ESP_LOG_ERROR) { esp_log_write(ESP_LOG_ERROR, "iap", "\033[0;" "31" "m" "E" " (%d) %s: " "iap_finish: esp_ota_end failed (%d)!" "\033[0m" "\n", esp_log_timestamp(), "iap", result); };
            return -1;
        }

        result = esp_ota_set_boot_partition(iap_state.partition_to_program);
        if (result != 0) {
            if (((esp_log_level_t) 3) >= ESP_LOG_ERROR) { esp_log_write(ESP_LOG_ERROR, "iap", "\033[0;" "31" "m" "E" " (%d) %s: " "iap_finish: esp_ota_set_boot_partition failed (%d)!" "\033[0m" "\n", esp_log_timestamp(), "iap", result); };
            return -1;
        }
    }

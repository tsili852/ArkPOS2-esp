# 1 "C:/esp/esp-idf/components/spiffs/spiffs/src/spiffs_cache.c"
...
# 9 "C:/esp/esp-idf/components/spiffs/spiffs/src/spiffs_cache.c" 2
...
s32_t spiffs_object_index_consistency_check(
    spiffs *fs);
# 10 "C:/esp/esp-idf/components/spiffs/spiffs/src/spiffs_cache.c" 2
...
static s32_t spiffs_cache_page_free(spiffs *fs, int ix, u8_t write_back) {
  ...
  if (cache->cpage_use_map & (1<<ix)) {
    if (write_back &&
      ...
      ;
      res = (fs)->cfg.hal_write_f((fs), (( ((fs)->cfg.phys_addr) + (cp->pix) * ((fs)->cfg.log_page_size) )), (((fs)->cfg.log_page_size)), (mem));
    }
...
s32_t spiffs_phys_rd(
  ...
  if (cp) {
    ...
    do{memcpy((dst),(&mem[( ((addr) - ((fs)->cfg.phys_addr)) % ((fs)->cfg.log_page_size) )]),(len));}while(0);
  } else {
    if ((op & (3<<0)) == (1<<0)) {

      return (fs)->cfg.hal_read_f((fs), (addr), (len), (dst));
    }
    ...
    if (cp) {
      ...
      s32_t res2 = (fs)->cfg.hal_read_f((fs), (addr - ( ((addr) - ((fs)->cfg.phys_addr)) % ((fs)->cfg.log_page_size) )), (((fs)->cfg.log_page_size)), (((u8_t *)(&((cache)->cpages[(cp->ix) * (sizeof(spiffs_cache_page) + ((fs)->cfg.log_page_size))])) + sizeof(spiffs_cache_page))))
      ...
      s32_t res2 = (fs)->cfg.hal_read_f((fs), (addr), (len), (dst));
      if (res2 != 0) {
...
s32_t spiffs_phys_wr(
  ...
  if (cp && (op & (7<<2)) != (6<<2)) {
    ...
    if ((op & (7<<2)) == (0<<2) &&
      ...
      spiffs_cache_page_free(fs, cp->ix, 0);
      return (fs)->cfg.hal_write_f((fs), (addr), (len), (src));
    }

    u8_t *mem = ((u8_t *)(&((cache)->cpages[(cp->ix) * (sizeof(spiffs_cache_page) + ((fs)->cfg.log_page_size))])) + sizeof(spiffs_cache_page));
    do{memcpy((&mem[( ((addr) - ((fs)->cfg.phys_addr)) % ((fs)->cfg.log_page_size) )]),(src),(len));}while(0);
...
void spiffs_cache_init(spiffs *fs) {
  ...
  do{memcpy((fs->cache),(&cache),(sizeof(spiffs_cache)));}while(0);
